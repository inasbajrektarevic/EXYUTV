...
class PackageEditScreen extends StatefulWidget {
  static const String routeName = "package/edit";
  final int id;
  const PackageEditScreen({super.key, required this.id});
...
class _PackageEditScreenState extends State<PackageEditScreen> {
  ...
  Future<void> loadData() async {
    loading = true;
    Package? item = await _packageProvider?.getById(widget.id, null);
    setState(() {
      this.data = item!;
      ...
      if (data != null) {
        ...
        iconUrlController.text = data.iconUrl ?? '';
        ...
        if (data.status != null) {
          _selectedStatus = statuses.firstOrNull((x) => x.key == data.status);
        }
        if (data.channelCategories != null) {
          selectedChannelCategories = data.channelCategories!
              .map((category) => category.channelCategoryId)
              .toList();
        } else {
  ...
  Widget build(BuildContext context) {
    return MasterScreenWidget(
        child: Scaffold(
            appBar: AppBar(
              title: Text("Izmjena podataka o paketu"),
            ),
            body: loading
                ? Center(
                          ...
                          children: [
                            ExpansionTileCard(
                              ...
                              children: [
                                SizedBox(
                                  height: 300,
                                  child: channelCategories.isNotEmpty
                                      ? ListView.builder(
                                          itemCount: channelCategories.length,
                                          itemBuilder: (context, index) {
                                            final category =
                                                channelCategories[index];
                              ...
                              onPressed: () async {
                                if (_formKey.currentState!.validate()) {
                                  final Map<String, dynamic> formData = {
                                    ...
                                    'channelCategorieIds':
                                        jsonEncode(selectedChannelCategories),
                                  };
                                  if (_imageBytes != null) {
                                    formData['file'] =
                                  ...
                                  bool? result = await _packageProvider
                                      ?.updateFormData(formData);
                                  showCustomToast(
                                    context,
                                    "Paket uspješno izmjenjen",
                                    Colors.green,
                                  );
                                  ...
                                  showCustomToast(
                                    context,
                                    "Greška prilikom izmjene podataka o paketu",
                                    Colors.green,
                                  );
                                }
                              },
